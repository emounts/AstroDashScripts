...
public class ProgressBarManager : MonoBehaviour
{
    ...
    [Header("UI Offsets (normalized, 0.01 = 1% of bar height)")]
    [SerializeField] private float checkpointIconOffsetNormalized = 0.03f; // tweak to taste
    ...
    // Cache for final, locked-in normalized positions
    ...
    private const float EARTH_ICON_OFFSET = -0.06f; // 3% below actual position to appear below rocket
    private const float PLANET_ICON_OFFSET = 0f; // Keep planets aligned to their true intercept point
        ...
        foreach (var obj in worldObjects)
        {
            ...
            if (!spawnedIcons.ContainsKey(obj))
            {
                ...
                if (rt != null)
                {
                    ...
                    // Also strictly disable the GameObject if not Earth
            ...
            // Disable any Canvas sorting overrides
            ...
            else if (obj is CelestialObjectController planetObj)
            {
                // Show planet icon when the planet has actually appeared in the world.
                // Once it has appeared, keep it visible on the progress bar.
                shouldShow = visibleIcons.Contains(obj) || planetObj.IsWorldVisible;
            }

            // --- Positioning ---
            // We only lock in (cache) positions when the icon should show.
            if (shouldShow)
            {
                ...
                else
                {
                    ...
                    {
                        // Only lock in a planet's UI position once it has actually appeared in-world.
                        // This prevents every planet from caching the same early off-screen placement.
                        bool haveSnapshot = planet.HasAppearSnapshot;

                        float rocketYAtAppear;
                        float planetYAtAppear;

                        if (haveSnapshot)
                        {
                            rocketYAtAppear = planet.AppearRocketY;
                            planetYAtAppear = planet.AppearPlanetY;
                        }
                        else
                        {
                            // If we don't have a snapshot yet, do not cache a position.
                            // Planet icon should not be visible until snapshot exists anyway,
                            // but this keeps us safe from race conditions.
                            normalizedPos = 0f;
                        ...
                        // If f >= 1, the planet moves faster than or equal to the rocket upward speed, so no intercept from below.
                        if (f < 0.99f)
                        {
                            // InterceptY = R0 + (P0 - R0) / (1 - f)
                            float dist = planetYAtAppear - rocketYAtAppear;
                            predictedY = rocketYAtAppear + (dist / (1f - f));
                        }
                        else
                        {
                            predictedY = planetYAtAppear;
                        }
                    }
                    else
                    {
                    ...
                    {
                        normalizedPos += checkpointIconOffsetNormalized;
                    }


                    // Cache so it doesn't change on respawn
